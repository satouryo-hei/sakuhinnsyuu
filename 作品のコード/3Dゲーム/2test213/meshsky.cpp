...
HRESULT CMeshSky::Init(D3DXVECTOR3 pos, D3DXVECTOR3 size, int meshX, int meshY)
{
	...
	*/
	...
	float fai = (D3DX_PI * 2) / 10 * 8;
	float fsita = (D3DX_PI * 2) / 10 * 8;
	...
	//			z = r *sin(θ)*cos(φ)
	//		*/
	//		float fai = (D3DX_PI * 2) / 32 * nCntY;
	//		float fsita = (D3DX_PI * 2) / 32 * nCntX;
	//		pVtx[nNum].pos = D3DXVECTOR3(50 * sin(fsita)*cos(fai), 50 * sin(fsita)*D3DX_PI / 2, 50 * sin(fsita)*cos(fai));
