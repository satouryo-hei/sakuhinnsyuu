...
HRESULT CMeshSky::Init(D3DXVECTOR3 pos, D3DXVECTOR3 size, int meshX, int meshY)
{
	...
	for (int nCntY = 0; nCntY < meshY + 1; nCntY++)
	{
		...
		{
			pVtx[nNum].pos = D3DXVECTOR3(cos((D3DX_PI * 2 / meshX) * nCntX) * (sin(((D3DX_PI * 2 / meshX) * nCntY) + ((D3DX_PI * 2 / meshX) * meshY)) * (size.x / 2.0f)),
	...
	(全ての頂点数)+(かぶり頂点数)
	((x+1)*(z+1))+((x+3)*(z-1)) */
	for (int nCnt1 = 0; nCnt1 < meshY; nCnt1++)
	{
		for (int nCnt2 = 0; nCnt2 < meshX + 1; nCnt2++)
		{
			//番号データの設定
			pIdx[(nCnt2 * 2) + 0 + (meshX + 2) * 2 * nCnt1] = (meshX + 1) + nCnt2 + (meshX + 1) * nCnt1;
			pIdx[(nCnt2 * 2) + 1 + (meshX + 2) * 2 * nCnt1] = 0 + nCnt2 + (meshX + 1) * nCnt1;
		}
	}
	for (int nCnt3 = 0; nCnt3 < meshY - 1; nCnt3++)
	{
		pIdx[((meshX + 1) * 2 + 0) * (nCnt3 + 1) + (2 * nCnt3)] = meshX + (meshX + 1) * nCnt3;
		pIdx[((meshX + 1) * 2 + 1) * (nCnt3 + 1) + (1 * nCnt3)] = meshX * 2 + 2 + (meshX + 1) * nCnt3;
	}
