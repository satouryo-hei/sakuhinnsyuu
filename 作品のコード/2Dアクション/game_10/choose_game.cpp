...
CPolygonEffect * CChoose_Game::m_pPolygonEffect = NULL;
...
HRESULT CChoose_Game::Init(D3DXVECTOR3 pos)
{
	...
	// エフェクトの生成処理の呼び出し
	m_pPolygonEffect = CPolygonEffect::Create(m_LodePos[m_nSituation],m_LodeSize[m_nSituation]);
...
void CChoose_Game::Update(void)
{
	...
	// 新しくゲームを始めたとき
	case CHOOSE_NEWGAME:				
		m_pPolygonEffect->SetColor(D3DXCOLOR(0.0f, 1.0f, 1.0f, 1.0f), CPolygonEffect::POLYGON_BACK);
	...
	case CHOOSE_CONTINUE:		
		m_pPolygonEffect->SetColor(D3DXCOLOR(1.0f, 0.0f, 1.0f, 1.0f), CPolygonEffect::POLYGON_BACK);
		if (CManager::GetInputKeyboard()->GetTrigger(DIK_RETURN) == true && m_bFade)
		{
			// つづかせる状態でゲームを始めさせる
			m_bNewGame = false;
	...
	case CHOOSE_TUTORIAL:			
		m_pPolygonEffect->SetColor(D3DXCOLOR(1.0f, 1.0f, 0.0f, 1.0f), CPolygonEffect::POLYGON_BACK);
		if (CManager::GetInputKeyboard()->GetTrigger(DIK_RETURN) == true && m_bFade)
		{	
			// チュートリアル画面に遷移
	...
	case CHOOSE_RESULT:		
		...
		m_pPolygonEffect->SetColor(D3DXCOLOR(0.0f, 1.0f, 0.0f, 1.0f), CPolygonEffect::POLYGON_BACK);
		if (CManager::GetInputKeyboard()->GetTrigger(DIK_RETURN) == true && m_bFade)
		{
			// リザルト画面に遷移
...
void CChoose_Game::SetPolygonColor(void)
{
	for (int nCnt = 0; nCnt < VTXBUFFER_MAX; nCnt++)
	{
		...
		// 頂点が0だった時
		case VTXBUFFER_ZERO:
			m_pPolygonEffect->SetVtxBuffColor(D3DXCOLOR(1.0f, 0.0f, 0.0f, 0.5f), CPolygonEffect::POLYGON_BEFORE, m_nRamNum);
			break;

		// 頂点が1だった時
		case VTXBUFFER_FAST:
			m_pPolygonEffect->SetVtxBuffColor(D3DXCOLOR(0.0f, 1.0f, 0.0f, 0.5f), CPolygonEffect::POLYGON_BEFORE, m_nRamNum);
			break;

		// 頂点が2だった時
		case VTXBUFFER_SECOND:
			m_pPolygonEffect->SetVtxBuffColor(D3DXCOLOR(0.0f, 0.0f, 1.0f, 0.5f), CPolygonEffect::POLYGON_BEFORE, m_nRamNum);
			break;

		// 頂点が3だった時
		case VTXBUFFER_THRD:
			m_pPolygonEffect->SetVtxBuffColor(D3DXCOLOR(1.0f, 1.0f, 0.0f, 0.5f), CPolygonEffect::POLYGON_BEFORE, m_nRamNum);
			break;

		// それ以外のとき
		default:
			m_pPolygonEffect->SetVtxBuffColor(D3DXCOLOR(0.0f, 0.0f, 0.0f, 0.5f), CPolygonEffect::POLYGON_BEFORE, m_nRamNum);
			break;
		}
		m_nRamNum++;
	}
